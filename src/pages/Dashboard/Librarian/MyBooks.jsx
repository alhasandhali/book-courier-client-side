...
const MyBooks = () => {
    ...
    const { data: books = [], isLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            // Assuming endpoint supports filtering by addedBy or email
            // If not, we might need to filter client side (less efficient)
            const res = await axiosSecure.get(`/books?addedBy=${user.email}`);
            return res.data;
        },
        enabled: !!user?.email
    });
                        ...
                        {books.map((book) => (
                            <tr key={book._id} className="border-b border-gray-50 hover:bg-gray-50/50">
                                ...
                                <td className="text-right">
                                    ...
                                    {/* <button className="px-3 py-1.5 border border-gray-200 rounded text-sm hover:bg-gray-50 transition-colors">
                                        {book.status === 'published' ? <i className="fas fa-eye-slash text-gray-500"></i> : <i className="fas fa-eye text-green-500"></i>}
                                    </button> */}
                                </td>
                            </tr>
                        ))}
