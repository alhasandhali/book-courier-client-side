    ...
    if (isError || !book) {
        return (
            <div className="min-h-[60vh] flex flex-col items-center justify-center text-center">
                ...
                <Link to="/" className="btn-primary rounded-full">Return Home</Link>
            </div>
        );
    }

    const renderRating = () => {
        const ratingVal = typeof book.rating === 'object' ? book.rating.average : book.rating;
        ...
        return (
            <div className="flex items-center gap-2 mb-4">
                <div className="flex items-center text-orange-400 gap-0.5">
                    {[...Array(5)].map((_, i) => (
                        <i key={i} className={`fa-star ${i < Math.floor(ratingVal) ? 'fa-solid' : 'fa-regular'} text-sm`}></i>
                    ))}
                </div>
                <span className="text-text-muted text-sm font-medium">({ratingVal || 0})</span>
                {ratingCount > 0 && <span className="text-text-muted text-sm border-l border-gray-300 pl-2 ml-1">{ratingCount} reviews</span>}
            </div>
        );
    };
        ...
        <section className="w-11/12 max-w-[1200px] mx-auto py-8 sm:py-12 md:py-16">
            <div className="text-sm breadcrumbs text-text-muted mb-6 sm:mb-10">
                <ul>
                    ...
                    <li className="font-semibold text-text-main overflow-hidden text-ellipsis whitespace-nowrap max-w-[200px]">{book.title}</li>
                </ul>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-14 items-start">
                ...
                <div className="flex flex-col h-full">
                    ...
                    <h1 className="font-serif text-3xl sm:text-4xl md:text-[2.75rem] leading-tight font-bold text-[#1a1a1a] mb-2">
                        {book.title}
                    </h1>

                    <p className="text-lg text-text-muted font-medium mb-3">By <span className="text-text-main">{book.author}</span></p>

                    {renderRating()}

                    <div className="flex items-center gap-4 mb-6 sm:mb-8 pb-6 sm:pb-8 border-b border-gray-100">
                        <div className="flex items-baseline gap-2">
                            <span className="font-serif text-3xl sm:text-4xl font-bold text-accent-gold">${book.price}</span>
                            {book.discount && book.discount > 0 && (
                                <span className="text-text-muted line-through text-lg">${book.price + book.discount}</span>
                            )}
                        </div>
                        {book.stock > 0 ? (
                    ...
                    <div className="mt-auto">
                        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 mb-6">
                            <div className="flex items-center border border-gray-200 rounded-lg px-2 h-[50px] w-fit">
                                ...
                                <span className="w-10 text-center font-bold text-text-main">{quantity}</span>
                                <button
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-text-muted text-xs">
                                            {book.delivery.deliveryTimeDays ? `Estimated ${book.delivery.deliveryTimeDays} days` : '3-5 business days'}
                                            • {book.delivery.deliveryCharge > 0 ? `$${book.delivery.deliveryCharge}` : 'Free'}
                                        </p>
                                    </div>
                                </div>
                                {book.delivery.availableCities && book.delivery.availableCities.length > 0 && (
                                    <div className="flex items-start gap-3 pt-2 border-t border-gray-200/50">
                                        ...
                                        <div>
                                            <p className="font-bold text-text-main mb-0.5">Available In</p>
                                            <p className="text-text-muted text-xs">
                        ...
                        {book.tags && book.tags.length > 0 && (
                            <div className="flex flex-wrap gap-2 mt-6">
                                {book.tags.map((tag, i) => (
                                    <span key={i} className="text-xs bg-gray-100 text-text-muted px-2.5 py-1 rounded hover:bg-gray-200 transition-colors cursor-default">#{tag}</span>
                                ))}
                            </div>
                        )}
                    </div>
